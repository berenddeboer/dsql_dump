...
export class TableExtractor {
  ...
  formatCreateTable(table: Table, clean: boolean = false): string {
    const lines: string[] = [];
    ...
    if (clean) {
      lines.push(`DROP TABLE IF EXISTS ${this.quoteIdentifier(table.name)};`);
      lines.push('');
    }
    ...
    if (table.comment) {
      lines.push('');
      lines.push(`COMMENT ON TABLE ${this.quoteIdentifier(table.name)} IS ${this.quoteLiteral(table.comment)};`);
    }

    // Add column comments if present
    for (const col of table.columns) {
      if (col.comment) {
        lines.push(`COMMENT ON COLUMN ${this.quoteIdentifier(table.name)}.${this.quoteIdentifier(col.name)} IS ${this.quoteLiteral(col.comment)};`);
      }
    }
